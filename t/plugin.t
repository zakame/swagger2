...
$t->get_ok('/api/pets')->status_is(500)->json_is('/errors/0/path', '$0.0.id')
  ->json_is('/errors/0/message', 'is missing and it is required')->json_is('/errors/1', undef)->content_is(123);
exit;
...
$t::Api::RES = [{id => 123, name => "kit-cat"}];
$t->get_ok('/api/pets?limit=foo')->status_is(400)->json_is('/errors/0/path', '$0.limit')
  ->json_is('/errors/0/message', 'string value found, but a integer is required')->json_is('/errors/1', undef);
